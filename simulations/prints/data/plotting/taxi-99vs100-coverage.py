...
num_nodes = 145
experiments = [
    ("taxi_normal_rw", "taxi - single random walker"),
]
...
thresholds = [100, 99]
labels = ["100% threshold", "99% threshold"]

for threshold, label_suffix, color in zip(thresholds, labels, colors):
    for folder, label in experiments:
        x, y = load_coverage_data(folder, num_nodes, label, threshold=threshold)
        plt.plot(x, y, label=f"{label} ({label_suffix})", linewidth=2.5, color=color)
...
plt.title("Random Walker Network Coverage (99% vs 100% threshold)", fontsize=14, weight='bold')
